...
def _available_in_python(python_exec: str, module: str) -> bool:
    result = subprocess.run(
        [python_exec, "-c", f"import importlib.util; raise SystemExit(0 if importlib.util.find_spec({module!r}) else 1)"],
        capture_output=True,
    )
    return result.returncode == 0


def _pip(package: str, log: Callable | None = None, python_exec: str | None = None) -> bool:
    py = python_exec or sys.executable
    if log:
        log(f"SYS: {Path(py).name} -m pip install {package} …")
    result = subprocess.run(
        [
...
def install_for_config(config: dict, log: Callable | None = None) -> None:
    ...
    for mod, pkg in unique:
        if not _available(mod):
