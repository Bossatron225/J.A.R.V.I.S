...
def _has_error(output: str) -> bool:
    ...
    return any(s in lower_output for s in error_signals)
...
Fixed code:"""
    ...
    response = model.generate_content(prompt)
    return _clean_code(_extract_text(response))
...
Updated code:"""
    ...
    try:
        response = model.generate_content(prompt)
        edited = _clean_code(_extract_text(response))
    except Exception as e:
...
Optimized code:"""
    ...
    try:
        response = model.generate_content(prompt)
        optimized = _clean_code(_extract_text(response))
    except Exception as e:
...
def _test_action(file_path, code, language, output_path, player) -> str:
    ...
    prompt = f"""You are an expert {lang} developer and QA engineer.
Write comprehensive unit tests for the following code using standard testing frameworks (e.g., unittest or pytest for Python, jest for JavaScript, etc.).
Return ONLY the test code — no explanation, no markdown, no backticks.
...
Unit tests:"""
    ...
    try:
        response = model.generate_content(prompt)
        test_code = _clean_code(_extract_text(response))
    except Exception as e:
        return f"Could not generate unit tests: {e}"

    if file_path:
        p = Path(file_path)
        save_path = p.parent / f"test_{p.name}"
    else:
        save_path = _resolve_save_path(output_path or "test_jarvis_code", lang)

    status = _save_file(save_path, test_code)
    print(f"[Code] ✅ Tests saved: {save_path}")
    return f"Unit tests generated. {status}\n\nPreview:\n{_preview(test_code)}"
...
def code_helper(
    ...
    parameters:
        ...
        file_path   : Path to existing file (edit / explain / run / build / optimize / test)
        code        : Raw code string (explain / optimize / test without a file)
