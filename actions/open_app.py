...
def _resolve_macos_app_path(app_name: str) -> Path | None:
    if _SYSTEM != "Darwin":
        return None
    ...
    for root in _MAC_APP_ROOTS:
        ...
        try:
            for candidate in root.rglob("*.app"):
                bundle = candidate.stem.lower().strip()
                compact = bundle.replace(" ", "").replace("-", "").replace("_", "")
                if bundle in variants or any(v in bundle or bundle in v for v in variants):
                    return candidate
                if compact in variants or any(v in compact or compact in v for v in variants):
                    return candidate
        except Exception:
            continue
