...
def _normalize_sessions(raw: object) -> list[dict]:
    ...
    for item in raw:
        ...
        entry: dict = {
            ...
            "summary": summary[:SESSION_TEXT_MAX],
        }
        language = str(item.get("language", "")).strip()
        if language:
            entry["language"] = language
        cleaned.append(entry)
    return cleaned[-SESSION_MAX:]
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                for key in base:
                    if key not in data:
                        data[key] = {}
                data["sessions"] = _normalize_sessions(data.get("sessions", []))
                return data
            return _empty_memory()
...
def _trim_to_limit(memory: dict) -> dict:
    ...
    # Drop oldest session summaries first to preserve stable personal memory.
...
def format_memory_for_prompt(memory: dict | None) -> str:
    ...
    id_fields = ["age", "birthday", "city", "job", "language", "school", "nationality"]
    ...
    sessions = _normalize_sessions(memory.get("sessions", []))
    if sessions:
        lines.append("")
...
def save_session_summary(summary: str, language: str = "") -> None:
    ...
    memory["sessions"] = sessions[-SESSION_MAX:]
    save_memory(memory)
    print(f"[Memory] 📝 Session saved ({entry['date']}): {summary[:60]}…")


def get_recent_sessions(limit: int = 3) -> list[dict]:
    """Return the most recent session summaries without consuming them."""
    if limit <= 0:
        return []
    memory = load_memory()
    sessions = _normalize_sessions(memory.get("sessions", []))
