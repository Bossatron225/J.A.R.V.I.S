                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                ...
                                self._log_turn_async("assistant", full_out)
                                if self._dashboard:
                                    asyncio.create_task(self._dashboard.broadcast({
...
class _HeadlessUI:
    ...
    def unlock_via_biometric(self, *, source: str = "web_dashboard_face_scan") -> tuple[bool, str]:
        """Clears the lock after the web dashboard's own SFace embedding match already
...
def _resolve_jarvis_mode() -> str:
    ...
    voice session — still linked to the VPS (memory sync, remote task servicing) if
    JARVIS_VPS_URL is set. Only an explicit JARVIS_MODE=worker (set by JarvisWorker.app's
    launch wrapper) selects worker mode. Everything else — including a bare
    `python main.py` run with JARVIS_VPS_URL sitting in the shell profile — defaults to
    interactive, so a manual launch always takes over from the background worker instead
    of silently colliding with it."""
    explicit = str(os.getenv("JARVIS_MODE") or "").strip().lower()
    return "worker" if explicit == "worker" else "interactive"
